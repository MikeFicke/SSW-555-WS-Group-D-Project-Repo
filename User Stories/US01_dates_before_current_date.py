    ...
    for individual in individual_dict.values():
        ...
        if birth_date and birth_date != "NA":
            ...
            if birth_date > current_date:
                print(f"ERROR: INDIVIDUAL: US01: {individual['ID']}: Birthday {birth_date} occurs in the future (Line {individual['Line']})")
        if death_date and death_date != "NA":
            death_date = datetime.datetime.strptime(death_date, "%Y-%m-%d").date()
            if death_date > current_date:
                print(f"ERROR: INDIVIDUAL: US01: {individual['ID']}: Death {death_date} occurs in the future (Line {individual['Line']})")
    for family in family_dict.values():
        ...
        if marriage_date and marriage_date != "NA":
            ...
            if marriage_date > current_date:
                print(f"ERROR: FAMILY: US01: {family['ID']}: Marriage date {marriage_date} occurs in the future (Line {family['Line']})")
        if divorce_date and divorce_date != "NA":
            ...
            if divorce_date > current_date:
                print(f"ERROR: FAMILY: US01: {family['ID']}: Divorce date {divorce_date} occurs in the future (Line {family['Line']})")
